...
### 💰 令牌 (Token) 消耗直接砍掉 80%-95%！

传统方式一次性加载整个项目文档，动辄 5-10 万令牌 (Token) 😱

我们采用**分层文档架构**：

| 对比项 | 令牌 (Token) 消耗 | 省了多少 |
|--------|-----------|---------|
...
助手: ## 登录实现
      ...
      3. 令牌 (Token) 生成 → `JwtUtil.createToken()`
...
助手: 找到 3 条相关问答：
      ...
      1. [feature] WiFi 怎么连接？| 2026-03-10
      2. [debug] WiFi 连接失败怎么办？| 2026-03-11
...
**按需生成**：详细文档只在需要时才生成，避免不必要的令牌 (Token) 消耗。

### 缓存机制

- ✅ Git 状态检测 - 有未提交 (Commit) 变更时更新
- ✅ 配置文件变更 - package.json 等被修改时更新
- ✅ 提交 (Commit) 变化 - HEAD commit 改变时更新
- ✅ TTL 过期 - 默认 24 小时
...
**新增配置项：**
- `feishu.doc_token` - 飞书文档令牌 (Token)
- `feishu.folder_token` - 飞书文件夹令牌 (Token)
- `feishu.wiki_token` - 飞书知识库令牌 (Token)
